...
for p_r in poss_reg:
    ...
    the_reg_object = RANSACRegressor(LinearRegression(), max_trials=10, min_samples=2,
                                     loss='absolute_loss', residual_threshold=10).fit(T_x, T_y)
    ...
    print(the_reg_object)
    print(the_reg_object.score(T_x, T_y))
    print("")
...
reg = RANSACRegressor(LinearRegression(), max_trials=10, min_samples=2,
                      loss='absolute_loss', residual_threshold=10).fit(T_n[best_p_r], T_y)
